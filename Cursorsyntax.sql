...
BEGIN
 OPEN EMP_CUR;
  LOOP
     FETCH EMP_CUR INTO FNAME,LNAME,DNAME,CITY,COUNTRY,EXPER;
     DBMS_OUTPUT.PUT_LINE(FNAME||' '||LNAME||' '||DNAME||' '||CITY||' '||COUNTRY||' '||EXPER);
...
DECLARE
 ...
 D.DEPARTMENT_NAME,
 E.SALARY,
 ROUND(MONTHS_BETWEEN(SYSDATE,HIRE_DATE)/12,2) AS T_EXP
 FROM EMPLOYEES E JOIN DEPARTMENTS D
 ON(E.DEPARTMENT_ID=D.DEPARTMENT_ID)
 WHERE D.DEPARTMENT_ID=&DID
...
BEGIN
  OPEN EMP_CUR;
  LOOP
     FETCH EMP_CUR INTO FNAME,LNAME,DNAME,SAL ,EXPER;
     DBMS_OUTPUT.PUT_LINE(FNAME||' '||LNAME||' '||DNAME||' '||SAL||' '||EXPER);
  EXIT WHEN EMP_CUR%NOTFOUND;
  END LOOP;
  CLOSE EMP_CUR;
END;
...
DECLARE
 ...
 SELECT *FROM(
 SELECT FIRST_NAME,LAST_NAME,SALARY,
 DENSE_RANK() OVER(ORDER BY SALARY DESC) AS RNKL
 FROM EMPLOYEES
 )WHERE RNKL<4; 
...
BEGIN
 OPEN EMP_CUR;
  LOOP
     FETCH EMP_CUR INTO FNAME,LNAME,SAL,RK;
     DBMS_OUTPUT.PUT_LINE(FNAME||' '||LNAME||' '||SAL||' '||RK);
     EXIT WHEN EMP_CUR%NOTFOUND;
  END LOOP;
 CLOSE EMP_CUR;
END;
